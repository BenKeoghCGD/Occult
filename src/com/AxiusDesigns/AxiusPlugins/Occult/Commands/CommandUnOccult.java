...
public class CommandUnOccult implements CommandExecutor {
    ...
    public CommandUnOccult(Occult occult, HashMap<String, String> messages) {
        this.plugin = occult;
        this.messages = messages;
    }

    public boolean onCommand(CommandSender sndr, Command command, String s, String[] args) {
        if(!sndr.hasPermission("Occult.Show")) return false;
        if(args.length < 1) {
        ...
        else
        {
            ...
            if(!plugin.checkName(args[0])) {
                sndr.sendMessage(parseString(messages.get("player-not-hidden").replaceAll("%player%", args[0])));
                return false;
            }
            else {
                Collection<? extends Player> players = Bukkit.getOnlinePlayers();
                EntityPlayer playersNMS = ((CraftPlayer) Bukkit.getPlayer(args[0])).getHandle();
                PacketPlayOutPlayerInfo packet = new PacketPlayOutPlayerInfo(PacketPlayOutPlayerInfo.EnumPlayerInfoAction.ADD_PLAYER, playersNMS);
                for(Player player : players) {
                    ((CraftPlayer) player).getHandle().playerConnection.sendPacket(packet);
                }
                sndr.sendMessage(parseString(messages.get("player-shown").replaceAll("%player%", args[0])));
                plugin.removeName(args[0]);
                return true;
            }
        }
    }
